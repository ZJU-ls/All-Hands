...
**R6 · Checkpoint writes 落在独立的 SQLite 文件(`backend/data/checkpoints.db`),与 `app.db` 同目录但分文件。**
- 分开的理由:
  - Checkpoint 写入密度高(每个 graph node transition 一次)· 和 app.db 的业务写混在一起会放大锁竞争
  - Backup / retention 策略不同 · app.db 是源真相、需要 PITR;checkpoints.db 是重放副本、可以定期截断
  - 一次 schema 迁移不会串扰另一侧(alembic 只管 app.db)
- 共通点:两个 DB 都开 WAL + busy_timeout · 都在 `settings.ensure_data_dir()` 下自动创建
- 原先草稿版的 R6 要求"同一 DB",实际落地发现 settings 已经有独立 `checkpoint_db_path` 字段 + Phase 1 实施后验证独立 DB 更合身,遂就地修正(2026-04-23 Accepted 时修订)
...
## R3 · Dual-SoT 消息恢复路径 · Delta-Send 契约

**日期:** 2026-04-23(晚)· **状态:** Accepted, impl pending(Phase A/B/C/D · 详见 `plans/2026-04-23-dual-sot-delta-send.md`)

R1 建好了 `execution/` 封装边界,R2 明确了"两个 SoT 是互补切片 · 不是同一状态两份";R3 补最后一块:**LLM 每轮到底收到什么**。前两条是空间契约(谁写哪里),R3 是时间契约(每轮怎么送)。

### Decision

**决策一 · Delta-Send · LLM 只收新 user 输入,不再重传全量历史**

`chat_service.send_message` 的调用面从"把 `[SystemMessage, *full_history]` 每轮扔进 `messages` 通道"变成"只送本轮 delta"。历史由 checkpointer 在 graph state 里天然延续,`create_react_agent(prompt=system_prompt)` 负责 system prompt(走 LangGraph prompt hook,不落状态通道 · 见 E26 修法)。

**决策二 · 冷启 bootstrap · 按 checkpointer 是否有 state 分路**

`chat_service` 在发消息前先 probe 当前 `thread_id` 在 checkpointer 里是否有 snapshot:

```python
has_prior_state = await self._has_checkpoint_state(conv_id)
if has_prior_state:
    # 热轮:只送新 user 消息 · checkpointer 里已有完整历史
    lc_messages = [{"role": "user", "content": user_msg.content, "id": user_msg.id}]
else:
    # 冷启(新会话 / 遗留会话首轮 / state 被清):送全量 MessageRepo 历史 bootstrap
    lc_messages = [以 msg.id 带 id 的全量历史]
```

冷启分支覆盖三种情况:① 新建会话第一轮;② pre-ADR-0014 的遗留会话首次"热续聊";③ 运维清过 `checkpoints.db` 的会话。都走同一条 bootstrap 路径 —— 不需要迁移脚本。

**决策三 · Stable Message Id · add_messages reducer 的去重锚**

所有送进 LangGraph 的 `HumanMessage` / `AIMessage` **必须携带稳定 id** —— 来自 `MessageRepo.Message.id`(每条 Message 在 MessageRepo 落库时就有 UUID)。LangGraph 的 `add_messages` reducer **按 id dedup**,stable id 让冷启 bootstrap 分支的重复注入在 reducer 层自然合并,不会二次爆炸。

上游 `SystemMessage` 走 `create_react_agent(prompt=...)` 这条专用通道,不进 messages 通道,不需要 id(E26 已定性)。

**决策四 · Compaction 不扰 LLM 上下文 · dual-SoT 的核心承诺**

`chat_service.compact(conv, keep_last=N)` 仍然 drop MessageRepo 的老行 + 插一条 summary 行。**但不动 checkpointer state**。这意味着:

- 前端看到"早期消息被折叠成 summary"(用户账本可整理)
- LLM 继续看到完整的 graph state 上下文(短期记忆不受扰)

这是 dual-SoT 设计最值钱的那个差:compaction 过去是"对 LLM 撒谎说它忘了",以后是"给用户整理视图,LLM 该记得还记得"。等到 LLM context 真的逼近窗口,再单独设计 "checkpointer-side compaction" 策略(v2 follow-up)。

### Rationale

- **对齐 R2 的设计意图** · R2 定义了"MessageRepo 是账本 / Checkpointer 是 graph 内部态",但 Phase 1-4 落地时还在每轮全量 replay —— 意图和实现脱节。R3 把意图落到调用面
- **修 E26 的根本病根** · E26 是"SystemMessage 重复注入"的表面症状;底层病根是"每轮把可以自然延续的状态当输入重发"。Hotfix(`prompt=` 参数)解了 SystemMessage 那条,R3 解掉 user/assistant messages 那条,从此整条 messages 通道都遵守"只送 delta"契约
- **支撑"整理账本但不扰 LLM"的产品价值** · compaction / summary / 审计视图这些未来会长的能力,都建立在"用户视图可变形 / LLM 记忆稳定"的解耦上 · R3 是那块基石

### Consequences

**正向:**
- checkpointer state 的 `messages` 通道长度 ≤ N·2 + O(tool_messages) · 不再 N² 爆
- 消息总字节传输下降(热轮只送 1 条 · 不再每轮重发前面所有)
- compaction 对 LLM 透明 · 产品侧可以放心做激进折叠
- `thread_id` 从"被动 key"彻底变成"主动 resume 载体" · 和 Claude Code `--resume <session>` 语义更靠近

**负向 / 需要守护:**
- `chat_service` 新增一个 checkpointer 探测点(`_has_checkpoint_state`) · 失败时 except 兜底返回 False · 降级回冷启全量,不会把流程卡死
- `api/deps.py` 要把 checkpointer 单例既注给 `AgentRunner` 也透给 `chat_service` · DI 面略扩
- `SkillRuntime` 持久化继续走 ADR 0011 的 `SkillRuntimeRepo`,**不跟** checkpointer 合流(R2 / R4 不变)
- R1 的 import-linter 规则继续守:`services/chat_service.py` 可以 `await checkpointer.aget_tuple(...)` 是因为 checkpointer 的**实例**经 DI 注入,但**类型** import 仍只在 `execution/` 侧 —— 需要在 probe helper 用 `BaseCheckpointSaver` 的 Protocol 型注释或 `TYPE_CHECKING` 导入,避免把 `langgraph.checkpoint.*` 拉进 `services/`
- 遗留会话首轮自动 bootstrap · 代码里就一个 `if not has_state: full_history` 分支 · 无需迁移脚本,但要在测试里显式覆盖(Phase C)

### Alternatives rejected

- **把 checkpointer 当唯一 SoT(丢弃 MessageRepo)** · 前端 `/messages` 拿不到 render_payloads / tool_calls 持久化结构(E24 修过) · 破 R2 · 拒
- **继续每轮全量,靠 `add_messages` 按 id dedup 兜底** · 即使不爆炸,也白白发一遍网络 + checkpointer sqlite 写放大 · 不符合"checkpointer 是短期记忆 · MessageRepo 是账本"的语义分工 · 拒
- **用消息 content hash 作 id(替代 MessageRepo.id)** · 用户连发两遍"你好"会被 reducer 误当成同一条 dedup 掉 · 拒 · 必须用 MessageRepo 的 UUID(业务实体 id · 与内容无关)

### Related

- R1(import-linter / 封装边界) · R2(双 SoT 互补切片) · R4(checkpointer 不替代其他 repo)继续生效
- [E26 · 非连续 system message 爆炸](../../docs/claude/error-patterns.md#e26--langgraph-checkpointer--systemmessage-in-input--非连续-system-消息累积--provider-拒绝) · hotfix 解 SystemMessage 那条;R3 把 user/assistant 那条一起解干净
- [`plans/2026-04-23-dual-sot-delta-send.md`](../../plans/2026-04-23-dual-sot-delta-send.md) · Phase A(stable ids) / B(delta send) / C(cold start) / D(compaction 一致性) 的落地任务

### Regression defense

新增(随 Phase A-D 落地):

- [`backend/tests/integration/test_dual_sot_stable_ids.py`](../../backend/tests/integration/test_dual_sot_stable_ids.py) · 三轮对话 · checkpointer `messages` 通道按 msg.id 合并 · 长度 ≤ 2·N · 证明 add_messages reducer 的 dedup 生效(Phase A)
- [`backend/tests/integration/test_delta_only_send.py`](../../backend/tests/integration/test_delta_only_send.py) · 热轮 chat_service 给 runner 的 `lc_messages` 长度 = 1(只有新 user) · 冷启 = full history(Phase B)
- [`backend/tests/integration/test_cold_start_replay.py`](../../backend/tests/integration/test_cold_start_replay.py) · 遗留会话(MessageRepo 有 3 条 · checkpointer 空)首轮自动 bootstrap · 第二轮走 delta(Phase C)
- [`backend/tests/integration/test_dual_sot_delta_consistency.py`](../../backend/tests/integration/test_dual_sot_delta_consistency.py) · 4 轮后 MessageRepo 末尾 pair 与 checkpointer state 末尾 pair 等价 · compaction 后 LLM 上下文仍完整(dual-SoT 核心承诺)(Phase D)
