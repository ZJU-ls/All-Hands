...
def _model(mid: str, provider_id: str, name: str, enabled: bool = True) -> LLMModel:
    return LLMModel(id=mid, provider_id=provider_id, name=name, enabled=enabled)


class TestResolveEffectiveModel:
    def test_conv_override_wins_when_valid(self) -> None:
        providers = [
            _provider("p1", "OpenAI", kind="openai", is_default=True),
            _provider("p2", "CODINGPLAN", kind="aliyun"),
        ]
        models = [
            _model("m1", "p1", "gpt-4o-mini"),
    ...
    def test_employee_ref_used_when_no_override(self) -> None:
        providers = [_provider("p1", "OpenAI", is_default=True)]
        models = [_model("m1", "p1", "gpt-4o-mini")]
        out = resolve_effective_model(
            conv_override=None,
            employee_ref="OpenAI/gpt-4o-mini",
            providers=providers,
    ...
    def test_falls_through_when_model_not_in_registered_list(self) -> None:
        ...
        models = [_model("m2", "p2", "qwen3.6-plus")]
        out = resolve_effective_model(
    ...
    def test_openai_kind_passes_through_unregistered_model(self) -> None:
        ...
        providers = [_provider("p1", "OpenRouter", kind="openai", is_default=True)]
        models = [_model("m1", "p1", "gpt-4o-mini")]
        out = resolve_effective_model(
    ...
    def test_empty_registered_models_passes_through(self) -> None:
        ...
        providers = [_provider("p1", "OpenAI", kind="openai", is_default=True)]
    ...
    def test_disabled_provider_is_skipped(self) -> None:
        providers = [
            _provider("p1", "OpenAI", is_default=True, enabled=False),
            _provider("p2", "CODINGPLAN", kind="aliyun", default_model="qwen3.6-plus"),
        ]
        models = [_model("m2", "p2", "qwen3.6-plus")]
        out = resolve_effective_model(
            conv_override=None,
        ...
        # No is_default among enabled → fall back to first enabled.
        assert out.source == "global_default"
        assert out.provider.id == "p2"
