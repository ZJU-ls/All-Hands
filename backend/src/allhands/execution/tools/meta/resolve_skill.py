...
RESOLVE_SKILL_TOOL = Tool(
    ...
    name="resolve_skill",
    description=(
        "Activate one of the skills mounted on this employee. Adds the "
        "skill's tools and prompt fragment to the current conversation "
        "(next turn onwards). Idempotent — calling twice is a no-op. "
        "Pass skill_id (one of the IDs in 'Available skills' list)."
    ),
    input_schema={
        "type": "object",
        "properties": {
