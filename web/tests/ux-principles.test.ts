...
describe("P04 · 远程数据组件必须覆盖 loading / empty / error 三态(启发式)", () => {
  ...
  const allow = (file: string) =>
    ...
    file === "components/chat/MessageBubble.tsx" || // attachment metadata fetch is silent/background — bubble already lives inside a message that has its own L/E/empty states; thumbnail miss naturally falls through to no-render. P04 exempt.
    file === "components/gateway/ModelGenTestDialog.tsx" || // dialog has explicit running/error states (RunButton + ErrorOrEmpty) but per-mode panels render natively — empty before first run is the natural form-only state; P04 heuristic doesn't see RunButton/ErrorOrEmpty as "loading/error". exempt.
