# AGENTS.md · allhands Dev Contract
...
>
> 本次 v1 refresh 见 [ADR 0011](product/adr/0011-principles-refresh.md)(4 条 → 6 条 · 参考 Codex + LangGraph 核心抽象)。
...
- 参考:Codex `query()` while-true 主循环(`ref-src-Codex/V02 § 2.1`)· LangGraph graph-as-state-transform
...
- 参考:Codex skill 体系 descriptor + lazy body-load(`ref-src-Codex/V05 § 2.1-2.3`)
...
- 参考:Codex Task tool · LangGraph subgraph composition
...
- 参考:Codex permission mode · LangGraph `interrupt()` + human-in-the-loop(语义同构)
...
- 参考:LangGraph Checkpointer · Codex `--resume <session>`(基于消息表 replay)
...
# [allhands] recent context, 2026-04-22 11:35pm GMT+8
