...
#
# Grep-based hard checks for the three highest-priority CLAUDE.md § 3.5 violations:
...
#
# Also catches `dark:` parallel definitions (§ 3.5.2 forbids — use CSS vars instead).
#
...
section "1 · icon libraries (CLAUDE.md § 3.5.1 · zero tolerance)"
...
section "2 · raw tailwind colour classes (§ 3.5.2 · use token only)"
...
section "3 · parallel dark: definitions (§ 3.5.2 · use CSS vars)"
check "dark: class utilities" "\\bdark:(bg|text|border)-" "tsx,jsx"

section "4 · motion-heavy libs (§ 3.5.3 · banned)"
check "framer-motion" "from ['\"]framer-motion" "ts,tsx,js,jsx"
check "gsap" "from ['\"]gsap" "ts,tsx,js,jsx"

section "5 · interaction feedback violations (§ 3.5.3 · banned for hover/active)"
...
if [[ $FAIL -eq 0 ]]; then
    ...
    printf 'See CLAUDE.md § 3.5 for the three highest rules (no icon libs · ≤3 colour density · motion ≤ 2px).\n'
    exit 1
fi
